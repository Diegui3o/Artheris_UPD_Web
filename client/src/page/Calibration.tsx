...
import FlightAnalyticsPanel from "../components/FlightAnalyticsPanel";
import { useState, useRef, useEffect } from "react";
import type { AnglesData } from "../types/angles";
import QuickActions from "../components/QuickActions";

type AnglesMsg =
  | { type: "angles"; payload: AnglesData }
  | { type: "telemetry"; payload: AnglesData }
  | Record<string, unknown>;

function useAnglesStream(url = "ws://localhost:9001") {
  const [last, setLast] = useState<
    (AnglesData & { timestamp?: number }) | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        const msg = JSON.parse(ev.data) as AnglesMsg;
        if (
          msg &&
          typeof msg === "object" &&
          "type" in msg &&
          ...
          if (msg.type === "angles" || msg.type === "telemetry") {
            const payload = msg.payload as AnglesData;
            setLast({ ...payload, timestamp: Date.now() });
          }
        }
      } catch {
  ...
  return last; // puede ser undefined hasta que llegue el primer paquete
}

const Calibration: React.FC = () => {
  const anglesData = useAnglesStream();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg p-4 border border-gray-700/50 hover:border-blue-500/30 transition-all duration-200">
            ...
            <QuickActions wsUrl="ws://localhost:9001" />
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50 mt-8">
          ...
          <FlightAnalyticsPanel sample={anglesData} />
        </div>
      </div>
    </div>
  );
